...
Created from lein template: 
```
lein new re-frame widgets +10x +test
```

Modified to manage deps and sources with `lein`.
...
* [`src/widgets/`](src/widgets/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/widgets/core.cljs): contains the SPA entry point, `init`
* [`test/widgets/`](test/widgets/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `widgets` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/widgets/config.cljs) defaults to `true` in
...
```clj
(ns widgets.example
  (:require [widgets.config :as config])
